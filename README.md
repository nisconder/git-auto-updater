...
1. 解析当前仓库应使用的远端（优先匹配配置 URL，避免与 `origin` 关系不一致）
2. `git fetch <remote>` - 获取远程更新
3. 检测工作区是否干净（如有未提交改动或未跟踪文件会跳过）
4. `git merge --ff-only <remote>/<当前分支>` - 仅执行快进更新，避免覆盖本地工作

这个策略会优先保护本地未提交内容，不再使用强制重置和清理。
...
1. **数据安全**：检测到本地未提交更改或未跟踪文件时会自动跳过更新，避免误覆盖
2. **权限要求**：确保有访问远程仓库的权限
...
A: 可以。当前版本会在检测到本地未提交更改时自动跳过更新，不会强制覆盖。
